# -*- coding: utf-8 -*-
"""
Created on Fri Jul 17 21:17:14 2020
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for sentence in self.knowledge:
            if len(sentence.cells) == 0:
                ...
                #Check whether set is none type

                if type(newsafes) is set:
                    new_safes |= newsafes

                if type(newmines) is set:
                    new_mines |= newmines
        ...
        new_knowledge = []
        sentence_b=new_sentence
        for sentence_a in self.knowledge:
            
            if sentence_a.count==0 :
                self.knowledge.remove(sentence_a)
                
            elif sentence_a == sentence_b:
                break
    ...
    def make_random_move(self):
        ...
        random_move = (random.randrange(0,8),random.randrange(0,8))
        if len(self.moves_made) == 56:
           return None

        unsafe_moves = self.moves_made | self.mines

        while random_move in unsafe_moves:
            random_move = (random.randrange(0,8),random.randrange(0,8))
